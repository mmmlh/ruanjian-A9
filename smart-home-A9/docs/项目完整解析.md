...
| `SecureStorage.ets` | 基于 Preferences 保存 Token 和 AES Key |
| `ServerConfig.ets` | 集中配置服务器协议、主机、端口，并派生 HTTP/WebSocket 地址 |
...
客户端服务器连接信息统一位于 `common/ServerConfig.ets`。修改 `SERVER_PROTOCOL`、`SERVER_HOST` 或 `SERVER_PORT` 后，HTTP API、WebSocket 和桌面卡片会使用同一组地址；服务器地址不再写入设备偏好设置。
