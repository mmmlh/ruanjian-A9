...
def _sync_device_status(topic: str, payload):
    """Persist device state and MQTT availability reported by a device."""
    if isinstance(payload, str):
        try:
            payload = json.loads(payload)
        except json.JSONDecodeError:
    ...
    parts = topic.split("/")
    if len(parts) < 4 or parts[-1] not in {"status", "response", "sensor", "availability"}:
        return
    ...
    if suffix == "availability" and not isinstance(payload.get("online"), bool):
        return
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if device:
                try:
                    previous_status = json.loads(device["status_json"] or "{}")
                ...
                if suffix == "availability":
                    previous_status["online"] = payload["online"]
                    status = previous_status
                elif isinstance(previous_status.get("online"), bool):
                    status["online"] = previous_status["online"]
...
async def lifespan(app: FastAPI):
    ...
    mqtt_client.subscribe("home/#", on_mqtt_message)
...
def health():
    ...
    mqtt_ready = bool(mqtt_status.get("started") and mqtt_status.get("connected"))
    mqtt_dependency = {
        **mqtt_status,
        "status": "ok" if mqtt_ready else "unavailable",
    }
    ready = database_status["status"] == "ok" and mqtt_ready
    content = {
        "status": "ok" if ready else "degraded",
        "dependencies": {
            "database": database_status,
            "mqtt": mqtt_dependency,
        },
    }
