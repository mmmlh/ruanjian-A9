...
class TestDevices:
    ...
    def test_mqtt_availability_updates_online_state_without_losing_device_status(self, client, db):
        db.execute("UPDATE devices SET updated_at = ? WHERE id = 4", ("2000-01-01 00:00:00",))
        db.commit()

        on_mqtt_message(
            "home/livingroom/light/availability",
            {"online": True, "device_id": "light_004", "ts": 1},
        )

        row = db.execute("SELECT status_json, updated_at FROM devices WHERE id = 4").fetchone()
        ...
        on_mqtt_message(
            "home/livingroom/light/availability",
            {"online": False, "device_id": "light_004", "ts": 2},
        )

        status = json.loads(db.execute("SELECT status_json FROM devices WHERE id = 4").fetchone()["status_json"])
        assert status["power"] == "off"
